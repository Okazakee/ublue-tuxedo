#!/bin/bash
# Load Tuxedo modules for Secure Boot
# Called by systemd service
# Automatically handles kernel updates by copying and signing modules for current kernel
...
if [ ! -f "$KEY" ] || [ ! -f "$CRT" ]; then
    echo "MOK keys not found, skipping module loading"
    exit 0
fi
...
if [ -x "/usr/src/kernels/${KVER}/scripts/sign-file" ]; then
    ...
    echo "No signing tool available"
    exit 1
fi
...
# Always refresh for current kernel version (handles kernel updates)
if [ -d "$DKMS_MODULE_DIR" ]; then
    # Try to copy and sign all modules
